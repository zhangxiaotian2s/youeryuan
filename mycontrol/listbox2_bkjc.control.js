...
function lesseonCheck() {
	...
	this.listbox2_bkjc_id = parseInt(plus.storage.getItem('listbox2_bkjc_id'))||0;
...
P_type.geSendArrValue = function(editdata) {
	...
	if (editdata) {
		...
		prepareLessonsCheckId = self.listbox2_bkjc_id;
	}
...
P_type.ajaxSendCheckMES = function(editMES) {
	...
	mui.ajax(_sendurl, {
		...
		data: {
			jsonStr: _sendData,
			id: self.listbox2_bkjc_id
		},
		timeout: 5000,
		success: function(data) {
			self.wating.close();
			if (data.Success == 10000) {
				plus.storage.setItem('listbox2_bkjc_id', (data.RerurnValue).toString());
...
P_type.ajaxGetEditMes = function() {
	...
	self.wating = plus.nativeUI.showWaiting();
	mui.ajax(self.editmesurl + self.listbox2_bkjc_id, {
		type: 'get',
		dataType: 'json',
		timeout: 5000,
		success: function(data) {
