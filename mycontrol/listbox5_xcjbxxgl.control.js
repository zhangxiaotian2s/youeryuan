...
function systemPage(){
	...
	this.worklisturl="http://115.28.141.223:89/WebServices/KMService.ashx?Option=GetKMSchoolBusRegisterInfoListByOrg&Id="+this.kgid
...
P_type.addWorkList=function(data){
	...
	for(var i=0;i<data.length;i++){
		var a=i+1
		_html+='<tr><td>'+a+'</td><td >'+data[i].BusNumber+'</td><td>'+data[i].LicensePlate+'</td><td>'+data[i].MannedNumber+'</td><td><button class="mui-btn btns" id="'+data[i].BusId+'"  style="width:100%">查看</button></td></tr>'
	}
...
var _btns	=document.querySelectorAll('.btns')
	for(i=0;i<_btns.length;i++){
		_btns[i].addEventListener('tap',function(){
			...
			mui.openWindow({
					url: "../pages/listbox5_xcjbxxgl_content.html",
					id: "../pages/listbox5_xcjbxxgl_content.html",
					styles: {
						top: '0px',
						bounce: 'none'
					},
