...
function systemPage() {
	...
	this.listbox3_qtbgl_id = parseInt(plus.storage.getItem('listbox3_qtbgl_id')) || 0;
		...
		success: function(data) {
			...
			if (data.Success == 10000) {
				mui.alert('提交成功！', '提示', function() {
					plus.storage.setItem('listbox3_qtbgl_id', (data.RerurnValue).toString());
					mui.back();
				});
			}
		},
...
P_type.ajaxGetEditMes = function() {
	...
	self.wating = plus.nativeUI.showWaiting();
	mui.ajax(self.editmesurl + self.listbox3_qtbgl_id, {
		type: 'get',
		dataType: 'json',
		timeout: 5000,
		success: function(data) {
