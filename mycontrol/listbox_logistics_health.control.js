...
SecurityCheckForm.prototype.loadLastSubmittedDate = function() {
	...
	if(_view.action == 'edit'){
		var _this = this;
		_waiting = plus.nativeUI.showWaiting();
		_id = parseInt(plus.storage.getItem('key_logistics_health_id'), 10);
		console.log('_id:' +  plus.storage.getItem('key_logistics_health_id'));
		...
		success: function(data) {
			...
			if (data.Success == 10000) {
				mui.alert('提交成功！', '提示', function() {
					...
					plus.storage.setItem('key_logistics_health_id', data.RerurnValue.toString());
					mui.back();
				});
			}else{
