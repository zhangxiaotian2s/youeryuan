...
function kitchenCheck() {
	...
	this.listbox3_cfcgjc_id = parseInt( plus.storage.getItem('listbox3_cfcgjc_id'))
...
kitchenCheck.prototype.geSendArrValue = function(editmes) {
	...
	if (!editmes) {
		...
		_KitchenCheckId = self.listbox3_cfcgjc_id;
	}
	for (i = 0; i < _length; i++) {
	...
	mui.ajax(_sendurl, {
		...
		success: function(data) {
			...
			if (data.Success == 10000) {
				...
				plus.storage.setItem('listbox3_cfcgjc_id', (data.RerurnValue).toString());
...
kitchenCheck.prototype.ajaxGetEditMes = function() {
	...
	self.wating = plus.nativeUI.showWaiting();
	mui.ajax(self.editmesurl + self.listbox3_cfcgjc_id, {
		type: 'get',
		dataType: 'json',
		timeout: 5000,
		success: function(data) {
