...
function systemPage() {
	...
	this.listbox7_jfjc_id = parseInt(plus.storage.getItem('listbox7_jfjc_id')) || 0;
		...
		success: function(data) {
			...
			if (data.Success == 10000) {
				mui.alert('提交成功！', '提示', function() {
					plus.storage.setItem('listbox7_jfjc_id', (data.RerurnValue).toString());
					mui.back();
				});
			}
		},
...
P_type.ajaxGetEditMes = function() {
	var self = this;
	//	self.listbox7_jfjc_id = 1 //临时处理
	if (!self.listbox7_jfjc_id) {
	...
	self.wating = plus.nativeUI.showWaiting();
	mui.ajax(self.editmesurl + self.listbox7_jfjc_id, {
		type: 'get',
		dataType: 'json',
		timeout: 5000,
		success: function(data) {
